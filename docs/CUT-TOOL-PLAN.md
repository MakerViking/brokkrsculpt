...
- **Repeated-cut pool fragmentation — the known gotcha, now reachable from ordinary editing.** `BlockAllocator` never splits or merges blocks; a freed block is reusable only by a request rounding to the same granule count (`GRANULARITY = 256`); the bump `watermark()`, not `live()`, is what fails an allocation. A cut monotonically *shrinks* the meshes it touches, so every changed brick orphans a block in a large granule class and takes a fresh one off the bump. `MeshPool::reset` — the only cure — is reachable **only** from `rebuild_everything` (resample / import / open / re-orient) and from no editing operation. Trim thirty spikes off a scan in one session and the plausible outcome is `MESH POOL FULL` with `live` far below capacity, whose banner then advises "delete a body or resample coarser" — neither of which is what the user wants after thirty successful trims — with reopening the file as the only real route. Mitigation: phase 0's thirty-cut bench makes it measurable, phase 7 asserts a watermark bound, and the banner gains an actionable remedy (a rebuild-view action calling `reset` + `mark_everything_dirty`) instead of advice that does not apply. **Open question — now closed, 2026-09-01: no, a cut must not trigger it.** The inclination above was right and the reasoning is worth keeping. `remesh_dirty` is a synchronous, unbudgeted whole-document remesh inside `update`, the same shape of work `MOST_BRICKS_TO_WALK` (app.rs) already declines to spend 320 ms on merely to add five words to a status line; spending seconds of it uninvited, mid-gesture, is a worse surprise than the banner it would silence, and a threshold cannot pick a good moment while a user can. Shipped instead: `Brokkr::rebuild_view` behind a **Rebuild view** button on the banner, offered only in the FRAGMENTED state — on a genuinely full pool a rebuild is the same multi-second freeze and it refuses exactly the same bricks again, so there the existing "delete a body or resample coarser" is the whole answer and a button would be a trap. Two things learned building it. `rebuild_view` deliberately keeps solo where `rebuild_everything` drops it, because that function's four callers all replace the document and renumber its rows and nothing here does. And no wording may imply memory came back: `MeshPool::reset` empties the allocators over buffers it already holds and never drops a `BufferPair`, so what is recovered is allocatable pool space, not VRAM — a test pins that the status says neither "MB" nor "freed".
