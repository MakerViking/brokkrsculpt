...
Verified by running it, not by reading it. **667 tests pass, 69 of them the updater's own**, clippy is clean at `-D warnings`, and the crate count went 314 → 315 exactly as predicted.
...
**Amended 2026-09-01, after this failed on a real Windows machine.** Saying it in the status line was not enough, and the reason is a layering fact this section did not know: the welcome card carries an install button of its own and draws over a full-window scrim, returning before every other card in `view` — so an install started from that screen reported its one and only explanation into a line hidden behind the very screen the button is on. That is why the failure was reported more than once and the error text was never read by anyone. Three changes: the refusal is drawn on the welcome card itself (`Brokkr::update_failure`); every update outcome is appended to `update-log.txt` in the state directory and carried in Help > Report a bug, because a Windows release build has no console and until then a failed update left no trace anywhere once the window was shut (`update/journal.rs`); and the spawn is retried with backoff for two seconds (`apply::relaunch`), since nothing re-waited for antivirus after the new file landed at the target path. The budget is short on purpose — ten seconds reproduces the "froze solid, said nothing" symptom that already caused one wrong report, and the apply lock is released before the spawn, so a long retry races a user who has given up and launched the build by hand. **The attempt count, not the retry, is the deliverable**: this fixes an unconfirmed hypothesis, and "after one try" versus "after six tries" is what will finally distinguish a permanent Smart App Control block from the transient hold assumed here.
...
The exposure just dismissed shrinks further against a decision made under *Release pipeline*: build-stamped payload sets are retained on the release precisely so a rollback has something to name, so anyone who wants a recent old build of ours has five of them one click away. (Twenty were retained until 2026-08-31, when the count was cut to five — rollback goes back one build, not twenty, and sixty payload files were burying the three a person actually comes to download.) Be exact about what `.old` is, though, because two of the comforting things one wants to say about it are untrue: it is executable, and `~/.local/bin` is on `PATH`. What bounds it is that nothing invokes a leading-dot name, that the gates above have already refused any directory that is not user-private, and that the revert path checks its digest before renaming it back.
